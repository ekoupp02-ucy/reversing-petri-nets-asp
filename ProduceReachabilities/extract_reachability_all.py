...
def get_random_reachability_state(model_path, default_end_time, execution_mode):
    print(f"[START] Extracting reachability state from model='{model_path}' mode='{execution_mode}'")
    out_file = reachability_filename(model_path, execution_mode)
    print(f"[INFO] Output reachability file will be: {out_file}")

    if os.path.exists(out_file):
        msg = f"Reachability file already exists: {out_file}"
        print(f"[SKIP] {msg}")
        return "exists", out_file, None, None, msg

    end_time = extract_end_time(model_path, default_end_time)
    print(f"[INFO] Extracted end_time={end_time} from {model_path}")

    try:
        unique_id = uuid.uuid4().hex[:8]
        result_dir = os.path.join("results_parallel", unique_id)
        os.makedirs(result_dir, exist_ok=True)
        print(f"[INFO] Temporary grounding directory: {result_dir}")

        project_root = os.environ.get("PROJECT_ROOT")
        if not project_root:
            raise RuntimeError("PROJECT_ROOT env var is not set")

        enc_map = {
            "forward": os.path.join(project_root, "ASP_ENCODINGS/SIMPLIFIED", "forwardCycles.lp"),
            "nonCausal": os.path.join(project_root, "ASP_ENCODINGS/SIMPLIFIED", "nonCausalCycles.lp"),
            "causal": os.path.join(project_root, "ASP_ENCODINGS/SIMPLIFIED", "causalCycles.lp"),
            "backward": os.path.join(project_root, "ASP_ENCODINGS/SIMPLIFIED", "backwardCycles.lp"),
        }

        encoding_file = enc_map[execution_mode]
        print(f"[INFO] Using encoding file: {encoding_file}")

        if not os.path.exists(encoding_file):
            msg = f"Encoding file does not exist: {encoding_file}"
            print(f"[ERROR] {msg}")
            return "missing_encoding", None, None, None, msg
        ...
        print(f"[INFO] step_by_step_grounding returned: {result_file}")

        if not result_file:
            msg = "step_by_step_grounding returned None/False/empty"
            print(f"[ERROR] {msg}")
            return "grounding_empty", None, None, None, msg

        if not os.path.exists(result_file):
            msg = f"Grounding returned path but file does not exist: {result_file}"
            print(f"[ERROR] {msg}")
            return "grounding_missing_file", None, result_file, None, msg
        ...
        with open(out_file, "w") as f:
            f.write(f"% Empty reachability file due to exception: {msg}\n")
        return "exception", out_file, None, None, msg

    try:
        with open(result_file, "r") as f:
            print(f"[INFO] Reading grounded file: {result_file}")
            atoms = f.read().split()
    except Exception as e:
        msg = f"Failed while reading grounded file '{result_file}': {type(e).__name__}: {e}"
        print(f"[EXCEPTION] {msg}")
        return "read_failed", out_file, result_file, encoding_file, msg
    ...
    for a in atoms:
        ...
        if a.startswith("holds("):
            ...
            if not m:
                print(f"[WARN] Could not parse holdsbonds atom: {a}")
                continue
    ...
    print(f"[INFO] Number of extracted predicates for target_t={target_t}: {len(preds)}")
    ...
    msg = f"Successfully extracted reachability file: {out_file}"
    print(f"[SUCCESS] {msg}")
    return "success", out_file, result_file, encoding_file, msg
# -------------------------
# Main (sharded)
# -------------------------
...
def main():
    ...
    for lp in shard_items:
        ...
        while flag:
            status, out, full_path, mode, reason = get_random_reachability_state(
                lp, default_end_time, execution_mode
            )

            print(f"[RESULT] model={lp}")
            print(f"[RESULT] status={status}")
            print(f"[RESULT] out={out}")
            print(f"[RESULT] full_path={full_path}")
            print(f"[RESULT] mode={mode}")
            print(f"[RESULT] reason={reason}")

            if status == "success":
                print(f"[VALIDATE] lp={lp}")
                print(f"[VALIDATE] grounded={full_path}")
                print(f"[VALIDATE] encoding={mode}")

                ok = vr.main(lp, full_path, mode)
                print(f"[VALIDATE] result={ok}")

                if ok:
                    flag = False
                else:
                    print(f"[ERROR] Validation failed for lp={lp}")
                    retries += 1
                    if retries >= MAX_RETRIES:
                        print(f"[FAIL] Giving up after {MAX_RETRIES} retries: {lp}")
                        flag = False
                    else:
                        print(f"[RETRY {retries}/{MAX_RETRIES}]")

            elif status == "exists":
                print(f"[SKIP] Existing reachability file found, moving on: {out}")
                break

            elif status == "grounding_empty":
                retries += 1
                if retries >= MAX_RETRIES:
                    print(f"[FAIL] Giving up after {MAX_RETRIES} retries: {lp}")
                    bad_name = lp.replace(".lp", "_dont_use.lp")
                    os.rename(lp, bad_name)
                    flag = False
                else:
                    print(f"[RETRY {retries}/{MAX_RETRIES}] Retrying with different seed: {lp}")

            elif status in {"grounding_missing_file", "missing_encoding", "read_failed", "exception"}:
                print(f"[FAIL] {status}: {lp}")
                print(f"[FAIL] reason={reason}")
                break

            else:
                print(f"[FAIL] Unknown status '{status}' for model {lp}")
                break
